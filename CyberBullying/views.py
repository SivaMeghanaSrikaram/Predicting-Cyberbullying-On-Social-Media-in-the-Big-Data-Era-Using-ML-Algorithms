...
def PostSent(request):
    if request.method == 'POST' and request.FILES['t2']:
        file_path = 'model_and_corpus.pkl'
        with open(file_path, 'rb') as file:
                data = pickle.load(file)
        # Access the model and corpus
        ...
        current_directory = os.getcwd()
        filename = fs.save(current_directory+'/static/photo/'+str(myfile).replace(" ","_"), myfile)
        ...
        print(dataset, "<- dataset2 is")
        f = open("test.txt", "w")
        f.write(dataset)
        f.close()
        #test = pd.read_csv("test.txt")
        test = pd.read_csv("test.txt", encoding="utf-8")
...
def RunAlgorithm(request):
    ...
    if request.method == 'POST':
      ...
      if name == "Random Forest":
          cls = RandomForestClassifier(n_estimators=1,max_depth=1,random_state=None)
          cls.fit(X_train, y_train) 
          prediction_data = prediction(X_test, cls)
    ...
    #       output = 'Extreme Machine Learning Algorithm Output details<br/><br/>'+msg
      classifier = cls
      context= {'data':''+output}   
      # File path to save both objects
      file_path = 'model_and_corpus.pkl'
      # Save the model and corpus to a file
      with open(file_path, 'wb') as file:
            pickle.dump({"model": classifier, "corpus": corpus}, file)
